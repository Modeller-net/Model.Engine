...
{
    private static readonly Parser<char, string> s_signString
        = Char('-').ThenReturn("-")
            .Or(Char('+').ThenReturn("+"))
            .Or(Parser<char>.Return(""));

    private static readonly Parser<char, int> s_sign
        = Char('+').ThenReturn(1)
    ...
    public static Parser<char, int> UnsignedInt(int @base)
        => DigitChar(@base)
            .ChainAtLeastOnce<int, IntChainer>(c => new(@base))
            .Labelled($"base-{@base} number");

    private struct IntChainer(int @base) : IChainer<int, int>
    {
        private int _result = 0;

        public void Apply(int value)
        {
            _result = (_result * @base) + value;
        }
    ...
    public static Parser<char, long> UnsignedLong(int @base)
        => DigitCharLong(@base)
            .ChainAtLeastOnce<long, LongChainer>(c => new(@base))
            .Labelled($"base-{@base} number");

    private struct LongChainer(int @base) : IChainer<long, long>
    {
    ...
    public static Parser<char, double> Real { get; }
        = s_signString
            .Then(
                s_fractionalPart
                    .Or(Digit.SkipAtLeastOnce().Then(s_optionalFractionalPart)) // if we saw an integral part, the fractional part is optional
            )
            .Then(
                CiChar('e').Then(s_signString).Then(Digit.SkipAtLeastOnce())
