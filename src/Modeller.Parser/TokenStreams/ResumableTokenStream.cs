...
{
    private static readonly bool s_needsClear = RuntimeHelpers.IsReferenceOrContainsReferences<TToken>();
    private readonly ArrayPool<TToken> _pool;
    private readonly ITokenStream<TToken> _next;
    private TToken[]? _buffer;
    private int _bufferStart;  // amount of empty space at left-hand end of _buffer, aka index of first value
    ...
    public void Return(ReadOnlySpan<TToken> leftovers)
    {
        ...
        if (_bufferStart < leftovers.Length)
        {
            ...
            _pool.Return(_buffer, s_needsClear);
            _buffer = newBuffer;
