...
public struct PooledList<T> : IDisposable, IList<T>
{
    ...
    public void Dispose()
    {
        if (_items != null)
        {
            _arrayPool.Return(_items, NeedsClear);
        }
    ...
    [DoesNotReturn]
    private static void ThrowArgumentOutOfRangeException(string paramName) => throw CreateArgumentOutOfRangeException(paramName);
