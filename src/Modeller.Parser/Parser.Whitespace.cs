...
{
    public override unsafe bool TryParse(ref ParseState<char> state, ref PooledList<Expected<char>> expected, out Unit result)
    {
        const long space = ' ';
        ...
        while (chunk.Length == 32)
        {
            fixed (char* ptr = chunk)
            {
                for (var i = 0; i < 8; i++)
                {
                    if (*(long*)(ptr + i * 4) == fourSpaces) continue;
                    // there's a non-' ' character somewhere in this group of four
                    for (var j = 0; j < 4; j++)
                    {
                        if (char.IsWhiteSpace(chunk[i * 4 + j])) continue;
                        state.Advance(i * 4 + j);
                        return true;
                    }
                }
        ...
        {
            if (char.IsWhiteSpace(chunk[i])) continue;
            state.Advance(i);
            return true;
        }
