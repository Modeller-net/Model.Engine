...
using Config = Modeller.Configuration.Configuration;
...
public static class ParserExtensions
{
    ...
    {
        var expected = new PooledList<Expected<TToken>>(state.Configuration.ArrayPoolProvider.GetArrayPool<Expected<TToken>>());

        var result1 = parser.TryParse(ref state, ref expected, out var result)
            ? new(result)
            : new Result<TToken, T>(state.BuildError(ref expected));
