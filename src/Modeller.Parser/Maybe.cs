...
public readonly struct Maybe<T> : IEquatable<Maybe<T>>
{
    ...
    public Tu Match<Tu>(Func<T, Tu> just, Func<Tu> nothing)
    {
    ...
    public Maybe<Tu> Select<Tu>(Func<T, Tu> selector)
    {
        ArgumentNullException.ThrowIfNull(selector);

        return HasValue ? Maybe.Just(selector(_value)) : Maybe.Nothing<Tu>();
    }
    ...
    /// </summary>
    /// <param name="selector">A transformation function to apply to the contained value.</param>
    /// <typeparam name="Tu">The type of the resulting possibly-absent value.</typeparam>
    ...
    public Maybe<Tu> SelectMany<Tu>(Func<T, Maybe<Tu>> selector)
    ...
    /// <typeparam name="Tu">The type of the value to select.</typeparam>
    /// <typeparam name="Tr">The type of the resulting possibly-absent value.</typeparam>
    ...
    public Maybe<Tr> SelectMany<Tu, Tr>(Func<T, Maybe<Tu>> selector, Func<T, Tu, Tr> result)
    {
        ArgumentNullException.ThrowIfNull(selector);
    ...
    /// <returns>A <see cref="Maybe{U}"/> containing this <see cref="Maybe{T}"/>'s value casted to <typeparamref name="Tu"/>, if the <see cref="HasValue"/> property returns true, or <see cref="Maybe.Nothing{U}()"/>.</returns>
    public Maybe<Tu> Cast<Tu>()
        => HasValue ? Maybe.Just((Tu)(object)_value!) : Maybe.Nothing<Tu>();

    /// <summary>
    /// Cast the value contained in the <see cref="Maybe{T}"/> to the specified result type, or return <see cref="Maybe.Nothing{U}()"/> if the contained value is not an instance of <typeparamref name="Tu"/>.
    /// </summary>
    /// <typeparam name="Tu">The type to cast the contained value to.</typeparam>
    /// <returns>A <see cref="Maybe{U}"/> containing this <see cref="Maybe{T}"/>'s value casted to <typeparamref name="Tu"/>, if the <see cref="HasValue"/> property returns true and the contained value is an instance of <typeparamref name="Tu"/>, or <see cref="Maybe.Nothing{U}()"/>.</returns>
    public Maybe<Tu> OfType<Tu>()
        => HasValue && _value is Tu ? Maybe.Just((Tu)(object)_value) : Maybe.Nothing<Tu>();
